...
#define COMMON_CONTEXT(a, v,model,modelv) \
 VANILLA_CONTEXT(a, v,model), VECTOR_CONTEXT(a, v, modelv)

#define VANILLA_CONTEXT(a, v,model) \
  Eq a,     Show a,     Arbitrary a,     CoArbitrary a,     TestData a model , model  ~ a, EqTest a~ Property

#define VECTOR_CONTEXT(a, v,modelv) \
  Eq (v a), Show (v a), Arbitrary (v a), CoArbitrary (v a), TestData (v a) modelv , modelv ~ [a],  EqTest a ~ Property, V.Vector v a
...
testSanity :: forall a v model modelv  . (COMMON_CONTEXT(a, v,model,modelv)) => v a -> [Test]
testSanity _ = [
...
testPolymorphicFunctions :: forall a v model  modelv . (COMMON_CONTEXT(a, v,model,modelv), VECTOR_CONTEXT(Int, v,modelv)) => v a -> [Test]
...
testTuplyFunctions:: forall a v model modelva modelvaa modelvaaa . (COMMON_CONTEXT(a, v,model,modelva), VECTOR_CONTEXT((a, a), v,modelvaa), VECTOR_CONTEXT((a, a, a), v,modelvaaa)  ) => v a -> [Test]
...
testOrdFunctions :: forall a v model modelv . (COMMON_CONTEXT(a, v,model, modelv), Ord a, Ord (v a)) => v a -> [Test]
...
testEnumFunctions :: forall a v model modelv . (COMMON_CONTEXT(a, v,model,modelv), Enum a, Ord a, Num a, Random a) => v a -> [Test]
...
testMonoidFunctions :: forall a v model modelv . (COMMON_CONTEXT(a, v,model,modelv), Monoid (v a)) => v a -> [Test]
...
testFunctorFunctions :: forall a v model modelv . (COMMON_CONTEXT(a, v,model,modelv), Functor v) => v a -> [Test]
testFunctorFunctions _ = $(testProperties
  [ 'prop_fmap ])
  where
    prop_fmap :: P ((a -> a) -> v a -> v a) = fmap `eq` fmap

testMonadFunctions :: forall a v model modelv . (COMMON_CONTEXT(a, v,model,modelv), Monad v) => v a -> [Test]
...
testApplicativeFunctions :: forall a v model modelv . (COMMON_CONTEXT(a, v,model,modelv), V.Vector v (a -> a), Applicative.Applicative v) => v a -> [Test]
...
testAlternativeFunctions :: forall a v model modelv . (COMMON_CONTEXT(a, v,model,modelv), Applicative.Alternative v) => v a -> [Test]
...
testBoolFunctions :: forall v model modelv . (COMMON_CONTEXT(Bool, v,model,modelv)) => v Bool -> [Test]
testBoolFunctions _ = $(testProperties ['prop_and, 'prop_or])
  where
    prop_and :: P (v Bool -> Bool) = V.and `eq` and
    prop_or  :: P (v Bool -> Bool) = V.or `eq` or

testNumFunctions :: forall a v model modelv . (COMMON_CONTEXT(a, v,model,modelv), Num a) => v a -> [Test]
...
testNestedVectorFunctions :: forall a v model modelv . (COMMON_CONTEXT(a, v,model,modelv)) => v a -> [Test]
...
testGeneralBoxedVector :: forall a model modelv . (COMMON_CONTEXT(a, Data.Vector.Vector,model,modelv), Ord a) => Data.Vector.Vector a -> [Test]
...
testNumericBoxedVector :: forall a model modelv . (COMMON_CONTEXT(a, Data.Vector.Vector,model,modelv), Ord a, Num a, Enum a, Random a) => Data.Vector.Vector a -> [Test]
...
testGeneralPrimitiveVector :: forall a model modelv . (COMMON_CONTEXT(a, Data.Vector.Primitive.Vector,model,modelv), Data.Vector.Primitive.Prim a, Ord a) => Data.Vector.Primitive.Vector a -> [Test]
...
testNumericPrimitiveVector :: forall a model modelv . (COMMON_CONTEXT(a, Data.Vector.Primitive.Vector,model,modelv), Data.Vector.Primitive.Prim a, Ord a, Num a, Enum a, Random a) => Data.Vector.Primitive.Vector a -> [Test]
...
testGeneralStorableVector :: forall a model modelv . (COMMON_CONTEXT(a, Data.Vector.Storable.Vector,model,modelv), Data.Vector.Storable.Storable a, Ord a) => Data.Vector.Storable.Vector a -> [Test]
...
testNumericStorableVector :: forall a model modelv . (COMMON_CONTEXT(a, Data.Vector.Storable.Vector,model,modelv), Data.Vector.Storable.Storable a, Ord a, Num a, Enum a, Random a) => Data.Vector.Storable.Vector a -> [Test]
...
testGeneralUnboxedVector :: forall a model modelv . (COMMON_CONTEXT(a, Data.Vector.Unboxed.Vector,model,modelv), Data.Vector.Unboxed.Unbox a, Ord a) => Data.Vector.Unboxed.Vector a -> [Test]
...
testNumericUnboxedVector :: forall a model modelv . (COMMON_CONTEXT(a, Data.Vector.Unboxed.Vector,model,modelv), Data.Vector.Unboxed.Unbox a, Ord a, Num a, Enum a, Random a) => Data.Vector.Unboxed.Vector a -> [Test]
...
testTupleUnboxedVector :: forall a model modelv  . (COMMON_CONTEXT(a, Data.Vector.Unboxed.Vector,model,modelv), Data.Vector.Unboxed.Unbox a, Ord a) => Data.Vector.Unboxed.Vector a -> [Test]
